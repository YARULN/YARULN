			...
			&& (clientSd == -1) )
	{
		...
		if(-1 != clientSd){
			...
			if(!strncmp(buf,USHARE_HTTPD_SYN_HEADER_MSG, USHARE_HTTPD_SYN_HEADER_LEN)){
				...
				ret = recvSocketData(clientSd, (char*)&gUshareInitInfo, sizeof(INIT_SERVER_INFO), 5, 0);				
...
{	
	...
	while(1){
		...
		switch(reqInfo.cmdType){
			...
			case ADD_FOLDER:
				/* 
				printf("add folder begin with name:%s; path:%s\n",
					reqInfo.contentInfo.displayName, reqInfo.contentInfo.fullPath); 
				*/
